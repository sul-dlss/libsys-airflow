...
from plugins.folio.helpers import archive_artifacts, move_marc_files, process_marc, tranform_csv_to_tsv
...
def airflow_results(tmp_path_factory):
    ...
    test_instance = migration_results_dir / f"folio_instances_{dag_id}_bibs-transformers.json"
    test_instance.write("""[{ "id": "12345-6789"}]""")
    test_holdings = migration_results_dir / f"folio_holdings_{dag_id}_holdings-transformer.json"
    test_holdings.write("""[{ "id": "45678-9123"}]""")
    test_items = migration_results_dir / f"folio_items_{dag_id}_bibs-transformer.json"
