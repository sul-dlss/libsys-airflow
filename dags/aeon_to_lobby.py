...
def transform_data(*args, **kwargs):
    ...
    for aeon_user in aeon_users:
        ...
        user = {"CustomFields": [], "IsBlocked": False,
                "BlockedReason": None, "ReusableBadgeBarcode": None}
        ...
        user["CustomFields"].append({"Name": "Address (Street)", "Value": aeon_user["adddress"]})
        ...
        user["CustomFields"].append(
            {"Name": "Zip code", "Value": aeon_user["zip"] })
        user["CustomFields"].append({ "Name": "Country", "Value": aeon_user["country"] })
        user["CustomFields"].append(
            {"Name": "Desired Library or Collection", "Value": aeon_user["Special Collections"]})
...
with DAG(
  ...
  aeon_user_data = PythonOperator (
      task_id="get_user_data_from_aeon",
      python_callable=user_data
  )

  transform_to_lobby_data = PythonOperator (
      task_id="transform_aeon_data_to_lobby_json",
      python_callable=transform_data
  )

  post_to_lobbytrack = PythonOperator (
      task_id="post_to_lobbytrack",
      python_callable=lobby_post
  )
