...
class HoldingsMarcTransformer(MigrationTaskBase):
    ...
    def do_work(self):
        files = [
            ...
            if (self.folder_structure.legacy_records_folder / f.file_name).is_file()
        ]
        if not any(files):
            ret_str = ",".join(f.file_name for f in self.task_config.files)
            logger.info(f"files empty failed on {self.folder_structure.legacy_records_folder} for {ret_str}")
            raise TransformationProcessError(
