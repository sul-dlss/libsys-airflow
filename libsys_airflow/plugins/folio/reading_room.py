...
class ReadingRoomsData:
    ...
    def patron_group_lookup(self) -> dict:
        lookup = {}
        groups = folio_client().folio_get_all("groups", key="usergroups", limit=99)
        for g in groups:
            lookup[g['id']] = g['group']
    ...
    def reading_rooms_lookup(self) -> dict:
        lookup = {}
        rooms = folio_client().folio_get_all(
            "reading-room", key="readingRooms", limit=99
        )
        for r in rooms:
            lookup[r['name']] = r['id']
...
class FolioUser:
    ...
    def patron_group_name(self) -> str:
        ...
        return reading_rooms_data.patron_groups.get(self.patronGroup, "")

    @property
    def usergroup_name(self) -> str:
        reading_rooms_data = ReadingRoomsData()
        return reading_rooms_data.usergroups.get(
...
def retrieve_users_for_reading_room_access() -> list:
    ...
    from_date = params.get("from_date", yesterday)
    client = folio_client()
    users = client.folio_get_all(
        "users",
        key="users",
        query=f'updatedDate>"{from_date.strftime("%Y-%m-%dT00:00:00.000+00:00")}"',
        limit=99999,
    )
...
@task
def generate_reading_room_access(users: list) -> list:
    ...
    for user in users:
        ...
        patron_permissions = {"userId": folio_user.id, "permissions": []}
        ...
        for k, v in reading_rooms_config.items():
            ...
            if not any(ea.get('readingRoomName') == k for ea in existing_access):
                patron_permissions['permissions'].append(
                    {
                        "id": str(uuid.uuid4()),
        ...
        patron_permissions['permissions'].extend(existing_access)
