...
def _add_batch_log(**kwargs) -> int:
    """Placeholder to add record to batch completed table"""
    email = kwargs["email"]
    fund_id = kwargs["fund_id"]
    instance_count = kwargs["instance_count"]
    filename = kwargs["filename"]
    logger.info(
        f"Adds to batch log email: {email}, fund_id: {fund_id}, instances: {instance_count}, uploaded filename: {filename}"
    )
    return -1
...
def _trigger_add_979_dags(**kwargs):
    ...
    fund = kwargs["fund"]
    batch_log_id = kwargs["batch_log_id"]
    dag_payload = {instance_uuid: fund}
    logger.info(
        f"Triggers 979 DAG with payload {dag_payload} with batch_log_id: {batch_log_id}"
    )
...
class DigitalBookplatesBatchUploadView(AppBuilderBaseView):
    ...
    def trigger_add_975_dags(self):
        ...
        try:
            ...
            batch_log_id = _add_batch_log(
                email=email,
                fund_id=fund_db_id,
                instance_count=len(upload_instances_df),
                filename=raw_upload_instances_file.filename,
            )
            for row in upload_instances_df.iterrows():
                instance_uuid = row[1]
                _trigger_add_979_dags(
                    instance_uuid=instance_uuid, fund=fund, batch_log_id=batch_log_id
                )
