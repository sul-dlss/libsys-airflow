...
def test_get_inventory_audit_report(mock_dag_run, mock_file_system):
    ...
    exists_db_id = _add_record({
        ...
        "hrid": "a34914",
        "_version": "1"
    }, db_connection, FOLIONamespaces.instances.value)

    add_audit_log(exists_db_id, db_connection, AuditStatus.EXISTS.value)

    for missing_record in [
        {"id": "d0d0edbf-e997-4496-ba53-32ec16660889", "hrid": "a56678", "_version": "1"},
        {"id": "80e5a714-7b99-4662-8c7d-ef846076ace2", "hrid": "a3990135", "_version": "1"}
    ]:
        missing_db_id = _add_record(missing_record, db_connection, FOLIONamespaces.instances.value)
        add_audit_log(missing_db_id, db_connection, AuditStatus.MISSING.value)

    inventory_audit_report(airflow=mock_file_system[0],
                           iteration=mock_dag_run.run_id)
