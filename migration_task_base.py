...
class MigrationTaskBase:
    ...
    def setup_logging(self):
        ...
        logger2.info(f"Before data_issues in setup_logger")
        ...
        logger2.info("Before getting logger")
        logger = logging.getLogger()
        logger2.info(f"After setting logger {logger}")

        # logger.handlers = []
        logger2.info("Before setting formatter")
        # formatter = logging.Formatter(
        #     "%(asctime)s\t%(levelname)s\t%(message)s\t%(filename)s:%(lineno)d"
        # )

        # logger2.info(f"Before stream_handler")
        # stream_handler = logging.StreamHandler()
        # stream_handler.addFilter(ExcludeLevelFilter(25))
        # stream_handler.addFilter(ExcludeLevelFilter(26))
        # logger2.info(f"After stream_handler")
